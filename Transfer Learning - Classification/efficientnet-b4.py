...
class ClassificationNet(pl.LightningModule):
    def __init__(self, num_classes):
        ...
        # Load the efficientnet_b4 pretrained model as the backbone
        ...
        # Replace the classifier layer with a new one with the appropriate number of output classes
        # just change classifier layer to adjust for our own classes

        self.backbone.classifier = nn.Sequential(
                                   nn.Linear(in_features=1792, out_features=self.num_classes, bias=True),
                                )
