    ...
    def __getitem__(self, item):
        ...
        if self.input_channels == 1:
            input_image = np.array(Image.open(input_image_path).convert("RGB")) # I'm not sure. Backbone has 3 input channel
                                                                                # If you recode backbone in channel in lightning_model,
                                                                                # actually it is basic, you can use data that has 1 input channel
                                                                                # input_image = np.array(Image.open(input_image_path).convert("L"), dtype=np.float32)
